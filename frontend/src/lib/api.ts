...
function mapToUICandidate(sub: any, record: any): Candidate {
  ...
  return {
    id: sub.candidate_id,
    name: record?.full_name || "Unknown Candidate",
    title: sub.title || record?.current_title || "Unknown Title",
    company: record?.current_company || record?.profile?.current_company || "Unknown Company",
    trajectory,
    rank: sub.rank ?? 0,
    matchScore: matchScore,
    isLowMatch,
    whyHere: isLowMatch ? ["Low Match Score"] : ["Domain Affinity"],
...
export async function getLatestJobId(): Promise<string | null> {
  ...
  const res = await fetch(new URL('/jobs/', apiBase), { cache: 'no-store' });
...
export async function getCombinedShortlist(jobId?: string): Promise<Candidate[]> {
  ...
  const apiBase = getApiBase();
  const data = await fetchLatestRanking(jobId, apiBase);
...
export async function getRankingMetadata(jobId?: string) {
  ...
  if (!data || data.status !== 'completed') {
    return {
      ...DEFAULT_METADATA,
      jdTitle: data?.query_text || DEFAULT_METADATA.jdTitle,
    };
  }

  return buildRankingMetadata(data);
}
...
export async function compareCandidates(candidateIds: string[]) {
  const apiBase = getApiBase();
  const url = new URL('/candidates/compare', apiBase);
...
export async function searchCandidates(params: {q?: string, skills?: string, title?: string, experience_min?: number, experience_max?: number}) {
  const apiBase = getApiBase();
  const url = new URL('/candidates/search', apiBase);
