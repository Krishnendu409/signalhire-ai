...
print("Running Extensive Feature Influence & Consistency Audit...")
...
stress_tests = [
    ('Sales Manager', 'Information Retrieval Engineer'),
    ('Sales Manager', 'UI Engineer'),
    ('Sales Manager', 'Machine Learning Engineer'),
    ('Frontend Engineer', 'Revenue Operations Manager'),
    ('Frontend Engineer', 'Customer Success Lead'),
    ('Search Engineer', 'Revenue Operations Manager')
]

for jd_name, title_override in stress_tests:
    jd_hidden = copy.deepcopy(base_jds[jd_name])
    jd_hidden['title_terms'] = title_override.lower().split()
    feat_hid = extract_features(df, jd_hidden)
    
    # Override candidate title string dynamically to simulate hidden title
    feat_hid['current_title'] = title_override.lower()
    # Re-run extract logic manually for the modified title
    feat_hid['title_affinity'] = 1.0 # Perfect match with override
    
    r_hid = rank_features(feat_hid.copy())
    top100 = r_hid.head(100)
    
    results['Section3_ContradictionStress'][f"{jd_name} + {title_override}"] = {
...
for jd_name, jd_data in base_jds.items():
    feat_base = extract_features(df, jd_data)
    results['Section4_PenaltySimulation'][jd_name] = {}
    
    for pen in [0, -1, -2, -4, -8]:
        r_pen = rank_features(feat_base.copy(), penalty=pen)
        top100 = r_pen.head(100)
        
        results['Section4_PenaltySimulation'][jd_name][f"Penalty_{pen}"] = {
            'Honeypot_Pct': (top100['is_trap'].sum() / 100.0) * 100
        }

with open('audit_v2_results.json', 'w') as f:
