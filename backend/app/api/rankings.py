...
async def create_ranking(
    ...
    candidates = [
        c for c in candidate_result.scalars().all()
        if c.parsed_data and "error" not in c.parsed_data
    ]
    if not candidates:
        raise HTTPException(
            status_code=400,
            detail="No parsed candidates found for this job. Upload resumes and wait for parsing to complete.",
        )
    ranking = RankingJob(
        id=uuid.uuid4(),
        job_id=job.id,
